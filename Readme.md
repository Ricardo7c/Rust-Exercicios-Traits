...
2. No `main`, crie um vetor de *trait objects* `Vec<&dyn Forma>` para armazenar diferentes instâncias de `Circulo` e `Retangulo`.
...
2. No `main`, crie uma lista `Vec<&dyn Pagamento>` contendo uma instância de cada método de pagamento.
...
2. No `main`, crie um vetor `Vec<&dyn Sensor>` que armazene instâncias de cada tipo de sensor.
...
2. No `main`, crie uma lista `Vec<&dyn Animal>` para armazenar instâncias de `Leao` e `Macaco`.
...
2. No `main`, crie um vetor `Vec<&dyn Notificacao>` contendo uma instância de `Email` e uma de `SMS`.
